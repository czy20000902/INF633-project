...
public class FabricIK : MonoBehaviour
{
    ...
    void FastIK()
    {
        ...
        Vector3 vector_dis = target.position - bones[0].position;
        float dis = vector_dis.magnitude;
        // Change condition!
        if (dis > completeLength)
        {
            // bonesPositions[i] = ...
            Vector3 dir = vector_dis.normalized;
            for (int i=1; i< bones.Length; i++)
            {
                bonesPositions[i] = bonesPositions[i - 1] + dir * bonesLength[i - 1];
            }
        }
